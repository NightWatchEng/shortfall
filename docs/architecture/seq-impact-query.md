...
| 23 | CLI → on-call | `--format text` (default), `markdown` or `json`, all three from `engine/report`. In the two text renders the three money legs always carry their evidence tag, grounded or not; `CustomersLeg` has no `Evidence` field to carry; and coverage carries its `[trust]` tag **only when grounded** — which in an impact report it never is (step 20), so it prints `unavailable: <reason>` with no tag. An ungrounded money leg prints `none` with its caveat or note beside it, which is why the caveat is load-bearing rather than decorative. `RenderJSON` marshals the struct as it stands, empty maps included. The bare `n/a` form the `Unavailable` marker enables is in `report.Summary` — the one-line render for an incident tool's impact field, which `--format` does not reach |
