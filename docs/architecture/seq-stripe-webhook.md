...
webhooks never report. The adapter lives in `adapters/payment/stripe`
(wrapped client, webhook receiver, ledger reconciler).
...
sequenceDiagram
    ...
    WC->>ST: API call (metadata carries flow, entity, customer hash)
