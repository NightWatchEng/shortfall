...
| 8 | `emit` ⇢ export adapter | **Asynchronous.** `Record` does not reach the exporter; a background ticker (1 s by default) and `Close` call `Flush`, which calls `ExportMetrics` and `ExportEvents`. A drop at any point increments `biz_dropped_events_total{reason}`. The reasons `emit` produces — and ADR-0002's full enum, since its 2026-08-30 amendment retired the never-emitted `encode` — are `invalid`, `overflow` and `export` |
