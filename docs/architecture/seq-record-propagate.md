...
| 8 | `emit` ⇢ export adapter | **Asynchronous.** `Record` does not reach the exporter; a background ticker (1 s by default) and `Close` call `Flush`, which calls `ExportMetrics` and `ExportEvents`. A drop at any point increments `biz_dropped_events_total{reason}`. The reasons `emit` actually produces are `invalid`, `overflow` and `export`; ADR-0002's enum also lists `encode`, which no code path emits |
