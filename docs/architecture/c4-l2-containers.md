...
flowchart TB
    ...
    REG --- ENGINE
    QUERY --- ENGINE
    PAY -->|"biz.Outcomes, via your sink"| CODE
    ENGINE -->|"Compute()"| CLI
...
| `engine` → query adapter | The `query.Querier` interface — the only questions the engine may ask a backend. An adapter that cannot serve a verb returns `query.ErrUnsupported`, and the engine marks that leg unavailable with a reason rather than reporting zeros |
...
| `adapters/payment/stripe` → your code | Provider webhooks and wrapped-client responses become `biz.Outcome`s handed to the sink **your code** registers — your code is what calls `Record`. The adapter takes no dependency on `emit` (it imports only `biz` and stripe-go), which is what makes it a nested module you can skip |
