...
one `Compute()` call assembles the report on the way out.
...
flowchart LR
    ...
    subgraph engine["engine — one Compute() call"]
        ...
        COV["<b>coverage</b><br/><i>trust</i><br/>telemetry Σ at the value stage<br/>vs ledger (ADR-0016)"]
...
| `Request` → the four legs + coverage | One `Compute()` fans out to all of them; each answers independently and each carries its own evidence tag (`deterministic` / `estimate` / `trust`) |
| the four legs + coverage → `Report` | Assembly, not arithmetic across legs. The `Report` is the four legs plus the coverage trust line, a severity suggestion, and the provenance (`GeneratedAt`, `RegistryVersion`, `LibraryVersion`) a postmortem needs — never a single blended number |
