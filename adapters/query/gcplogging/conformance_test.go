...
func TestQuerierConformanceAgainstMemq(t *testing.T) {
	...
	// Guard the assertion itself: without these the loop above would pass
	// on a scenario that produced nothing at all.
	if !sawMoney {
...
// TestEventOrderIsFaithful pins that the adapter does not reorder or drop
// events on the way through: the decoded outcomes it hands the reference
// are the scenario's events, in the store's timestamp order.
func TestEventOrderIsFaithful(t *testing.T) {
	events, window := scenarioEvents()
	srv := newFakeBigQuery(t, entriesFor(events))
	q := srv.querier(t)

	got, err := q.fetch(context.Background(), query.EventQuery{Range: window})
	if err != nil {
		t.Fatal(err)
	}
	want := append([]biz.Outcome(nil), events...)
	// The fake serves rows in the order the fixture supplies them, which
	// entriesFor sorts by time — a real Log Analytics read is ORDER BY
	// timestamp, so this is the same contract.
	sortByTime(want)
	if len(got) != len(want) {
		t.Fatalf("decoded %d events, want %d", len(got), len(want))
	}
	if len(got) == 0 {
		t.Fatal("order assertion is vacuous: the scenario produced no events")
	}
	for i := range want {
