...
// and an in-memory ledger that serves as ground truth for the engine.
//
// Capture and settle are capacity-limited FIFO stages (per-minute worker
// capacity, fixed service delay); a queue-consumer-stall fault sets
// capacity to zero for its window, so a real backlog forms and drains at
// the stage's capacity afterwards. The api faults reject (5xx) or lose
// (latency abandonment) arrivals; an upstream blackout suppresses
// arrivals entirely, with an optional recovered fraction re-arriving
// after the outage. The ledger is omniscient: it records abandoned
// transactions and suppressed demand that real telemetry could never
// see — the asymmetry the engine's evidence labels are about.
//
// The simulation is discrete-event on a virtual clock (minute resolution,
// single-threaded): identical Config yields a byte-identical ledger on
// the same toolchain and architecture. Arrival sampling consumes
// math.Exp, which Go does not promise bit-identical across platforms, so
// golden expected values must be generated by CI (ubuntu/amd64), never on
// a laptop. All fixtures produced here are synthetic by construction —
...
// Segment is the customer segment (smb or enterprise).
type Segment string
...
type Txn struct {
	...
	// RecoveredFrom is the originating blackout's From instant, so a
	// recovery stays attributed to the incident that suppressed it —
	// even when the re-arrival was suppressed again by a later blackout
	// on its way back.
	Recovered     bool
...
type Config struct {
	...
	// There is no literal-zero form: a consumer that is down is a
	// queue-consumer-stall fault over the window, never a capacity of 0
	// (which would make the FIFO cursor spin forever).
...
// order for capture; capture completions for settle, non-decreasing
// given the disjoint stall intervals newStage guarantees by merging), so
// a single cursor suffices. schedule asserts that ordering precondition:
// a silent violation would misorder ground truth.
type stage struct {
	delay    time.Duration
	capacity int
...
func newStage(delayMin, capacity int, queue Queue, faults []FaultSpec) *stage {
	...
	// intervals the forward extension pass in schedule is exact.
